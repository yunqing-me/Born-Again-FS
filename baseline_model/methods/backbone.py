...
# Learnable Temperature used for Knowledge Distillation
class LearnableTemperature(nn.Module):
    maml = False

    def __init__(self, train_few_shot_params):
        super(LearnableTemperature, self).__init__()
        self.T = torch.nn.Parameter(torch.ones(size=(1, 1)) * 4,
                                    requires_grad=True)

    def forward(self, student_logits, teacher_logits, log=False):
        """
        :param teacher_logits
        :param student_logits
        :param log: log_softmax of student if true
        :return: pre-distill loss, symmetric kl_divergence
        """
        activation = softplus(self.T)
        if log:
            p1 = F.log_softmax(student_logits / activation, dim=1)
            q1 = F.softmax(teacher_logits / activation, dim=1)
            output1 = F.kl_div(p1, q1, reduction='batchmean') * (activation ** 2)
            return output1
        else:
            p2 = F.softmax(student_logits / activation, dim=1)
            q2 = F.log_softmax(teacher_logits / activation, dim=1)
            output2 = F.kl_div(q2, p2, reduction='batchmean') * (activation ** 2)
            return output2
...
# --- Simple ResNet Block for normal use---
